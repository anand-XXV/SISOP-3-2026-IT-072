...
**Komunikasi server ke client** menggunakan PID client sebagai `mtype`, sehingga setiap client hanya menerima pesan yang ditujukan untuknya.
...
Saat battle, terminal diubah ke raw mode (tanpa buffering, tanpa echo) agar input `a`/`u` bisa ditangkap langsung. Jika battle selesai atau program diinterrupt, `restore_mode()` dipanggil untuk mengembalikan terminal ke kondisi normal, mencegah terminal "rusak" setelah program selesai.
